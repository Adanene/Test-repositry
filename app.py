...
if ok:
    ...
    train_data, test_data = train_test_split(data, test_size=0.3, random_state=250)

    # Select the features and target variable
    features = ['B/T','Cb','MB/Disp' ]
    target = 'Inclinement'
...
if st.session_state.button_pressed:
        if jumlah_beban =="0" :
                ...
                new_test1 = pd.DataFrame({'B/T' :[BT] , 'Cb': [Cb],'MB/Disp' : [MD1], })
                predicted_Incline1 = best_model.predict(new_test1)
        
                new_test2 = pd.DataFrame({'B/T' :[BT], 'Cb': [Cb],'MB/Disp' : [MD2], })
                predicted_Incline2 = best_model.predict(new_test2)
        
                new_test3 = pd.DataFrame({'B/T' :[BT], 'Cb': [Cb],'MB/Disp' : [MD3],})
                predicted_Incline3 = best_model.predict(new_test3)
        
                new_test4 = pd.DataFrame({'B/T' :[BT], 'Cb': [Cb],'MB/Disp' : [MD4], })
                predicted_Incline4 = best_model.predict(new_test4)
        
                new_test5 = pd.DataFrame({'B/T' :[BT], 'Cb': [Cb],'MB/Disp' : [MD5], })
                predicted_Incline5 = best_model.predict(new_test5)
        
                new_test6 = pd.DataFrame({'B/T' :[BT], 'Cb': [Cb],'MB/Disp' : [MD6],})
                predicted_Incline6 = best_model.predict(new_test6)
        
                new_test7 = pd.DataFrame({'B/T' :[BT], 'Cb': [Cb],'MB/Disp' : [MD7],})
                predicted_Incline7 = best_model.predict(new_test7)
        
                new_test8 = pd.DataFrame({'B/T' :[BT], 'Cb': [Cb],'MB/Disp' : [MD8],})
